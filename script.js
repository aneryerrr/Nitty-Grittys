...
const IconInfo=(p)=>(<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className={iconCls} {...p}><circle cx="12" cy="12" r="9"/><path d="M12 8h.01M11 12h2v4h-2"/></svg>);

/* ---------- Constants & helpers ---------- */
const LOGO_SRC="/logo-ng.png";
const DEFAULT_SYMBOLS=["XAUUSD","US100","US30","EURUSD","BTCUSD","AUDCAD","USDCAD","USDJPY","GBPUSD"];
const DEFAULT_STRATEGIES=[
  "Trend Line Bounce","2 Touch Point Trend Line Break","3 / 3+ Touch Point Trend Line Break",
  "Trend Line Break & Re-test","Trend Continuation"
];
...
function computeDollarPnL(t,accType){
  // Prefer CSV-provided P&L if present
  if (typeof t.pnlProvided === "number" && isFinite(t.pnlProvided)) return t.pnlProvided;
...
function parseCSV(text){
  ...
  const rows=[]; let i=0, field="", inQ=false, line=[];
...
const nameMap = (h) => h.trim().toLowerCase().replace(/\s+/g,'');
function importMapHeaders(headerRow){
  const idx={};
  headerRow.forEach((h,ix)=>{
    const k=nameMap(h);
    if(['date'].includes(k)) idx.date=ix;
    if(['symbol','instrument','pair'].includes(k)) idx.symbol=ix;
    if(['side','direction','action'].includes(k)) idx.side=ix;
    if(['lotsize','lot','qty'].includes(k)) idx.lotSize=ix;
    if(['entry','entryprice'].includes(k)) idx.entry=ix;
    if(['exit','exitprice'].includes(k)) idx.exit=ix;
    if(['tp1','tp_1','takeprofit1','tp1exit'].includes(k)) idx.tp1=ix;
    if(['tp2','tp_2','takeprofit2','tp2exit'].includes(k)) idx.tp2=ix;
    if(['sl','stoploss','stop'].includes(k)) idx.sl=ix;
    if(['strategy'].includes(k)) idx.strategy=ix;
    if(['exittype','exitstatus'].includes(k)) idx.exitType=ix;
    if(['p&l','$','p&l($)','pnl','profit'].includes(k)) idx.pnl=ix;
    if(['p&l(units)','units','pnlunits'].includes(k)) idx.units=ix;
    if(['status','state'].includes(k)) idx.status=ix;
  });
  return idx;
}
function readCell(row,ix){
...
function Modal({title,children,onClose,maxClass}){
  return(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-3">
      <div className={`relative w-[95vw] ${maxClass||"max-w-3xl"} max-h-[85vh] overflow-auto bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl`}>
        <div className="sticky top-0 z-10 flex items-center justify-between px-4 py-3 border-b border-slate-700 bg-slate-800/95 backdrop-blur">
          <div className="font-semibold flex items-center gap-2"><IconSettings/>{title}</div>
...
/* ---------- Settings (account + customisation) ---------- */
function CustomizePanel({customSymbols,setCustomSymbols,customStrategies,setCustomStrategies,widgets,setWidgets,addCustomWidget}){
  const [symIn,setSymIn]=useState(""); const [strIn,setStrIn]=useState("");
  const [err,setErr]=useState("");

  const addSym=()=>{const v=symIn.trim(); if(!v) return; if(customSymbols.includes(v)) return; setCustomSymbols([...customSymbols,v]); setSymIn("")};
  const addStr=()=>{const v=strIn.trim(); if(!v) return; if(customStrategies.includes(v)) return; setCustomStrategies([...customStrategies,v]); setStrIn("")};
  const toggleWidget=(w)=>{const set = new Set(widgets||[]); set.has(w)?set.delete(w):set.add(w); setWidgets(Array.from(set))}

  const [cwTitle,setCwTitle]=useState("");
  const [cwKind,setCwKind]=useState("ExitTypeShare"); // supported kinds
  const [cwArg,setCwArg]=useState("TP"); // user typed
  const submitCustom=()=>{
    // Only allow supported kinds and known values
    const title=cwTitle.trim();
    ...
    if(cwKind==="StrategyWinRateFor" && !customStrategies.includes(cwArg)) {setErr("Strategy must exist in list.");return}
    setErr("");
    addCustomWidget({id:"CUST_"+Math.random().toString(36).slice(2), kind:cwKind, arg:cwArg, title});
    setCwTitle(""); setCwArg("TP");
  };
    ...
    <div className="space-y-8">
      <div>
        ...
        <div className="flex gap-2 mb-2">
          <input value={symIn} onChange={e=>setSymIn(e.target.value)} placeholder="Type symbol to add" className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2"/>
          <button onClick={addSym} className="px-3 py-2 rounded-lg bg-blue-600 hover:bg-blue-500">Add</button>
        </div>
        <div className="flex flex-wrap gap-2">{customSymbols.map(s=>(<span key={s} className="badge">
          <span>{s}</span>
          <button onClick={()=>setCustomSymbols(customSymbols.filter(x=>x!==s))} title="Remove">✕</button>
        </span>))}</div>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-2 mb-2">
          <input value={strIn} onChange={e=>setStrIn(e.target.value)} placeholder="Type strategy name to add" className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2"/>
          <button onClick={addStr} className="px-3 py-2 rounded-lg bg-blue-600 hover:bg-blue-500">Add</button>
        </div>
        <div className="flex flex-wrap gap-2">{customStrategies.map(s=>(<span key={s} className="badge">
          <span>{s}</span>
          <button onClick={()=>setCustomStrategies(customStrategies.filter(x=>x!==s))} title="Remove">✕</button>
        </span>))}</div>
      </div>
      ...
      <div>
        ...
        <div className="bg-slate-900/50 border border-slate-700 rounded-xl p-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            <input placeholder="Title (e.g., % TP closes)" value={cwTitle} onChange={e=>setCwTitle(e.target.value)} className="bg-slate-900 border border-slate-700 rounded-xl px-3 py-2"/>
            <select value={cwKind} onChange={e=>setCwKind(e.target.value)} className="bg-slate-900 border border-slate-700 rounded-xl px-3 py-2">
              <option value="ExitTypeShare">% of closes by Exit Type</option>
              <option value="StrategyWinRateFor">Win rate for Strategy</option>
            </select>
            <input placeholder="Arg (e.g., TP or Strategy name)" value={cwArg} onChange={e=>setCwArg(e.target.value)} className="bg-slate-900 border border-slate-700 rounded-xl px-3 py-2"/>
          </div>
          {err && <div className="text-red-400 text-sm mt-2">{err}</div>}
          <div className="text-right mt-2"><button onClick={submitCustom} className="px-3 py-2 rounded-lg bg-blue-600 hover:bg-blue-500">Add widget</button></div>
        </div>
      </div>
    </div>
  );
...
function SettingsPanel({name,setName,accType,setAccType,capital,setCapital,depositDate,setDepositDate,email,
  customSymbols,setCustomSymbols,customStrategies,setCustomStrategies,widgets,setWidgets,addCustomWidget,mode,onDone}){
  const [tab,setTab]=useState(mode==="setup"?"account":"account");
  return(
    <div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-6">
      ...
      {tab==="account" ? (
        ...
        <CustomizePanel
          ...
          customStrategies={customStrategies} setCustomStrategies={setCustomStrategies}
...
function TradeModal({initial,onClose,onSave,onDelete,accType,lastLotSize,customSymbols,customStrategies}){
  ...
  useEffect(()=>{
    const en = num(entry); const sll = num(sl);
    ...
    <Modal title="Calendar" onClose={onClose} maxClass="max-w-lg">
      ...
      {view==="day"&&(
        <div>
          ...
          {(trades.filter(t=>t.date===selectedDate)).length===0?(<div className="text-slate-400 text-sm">No trades this day.</div>):(
            <div className="space-y-2">{(trades.filter(t=>t.date===selectedDate)).map(t=>(<div key={t.id} className="bg-slate-900/50 border border-slate-700 rounded-xl p-3 flex items-center justify-between">
...
function NotesPanel({notes,setNotes,trades}){
  const [composeOpen,setComposeOpen]=useState(true); // open at entry
  const [selectedDate,setSelectedDate]=useState(todayISO());
  const [text,setText]=useState("");
  const [tradeIds,setTradeIds]=useState([]);
  const [editId,setEditId]=useState(null);

  const saveNote=()=>{const id=editId||Math.random().toString(36).slice(2);
    const arr=notes.slice();
    const rec={id,date:selectedDate,text,tradeIds};
    const idx=arr.findIndex(n=>n.id===id);
    if(idx>=0) arr[idx]=rec; else arr.unshift(rec);
    setNotes(arr); setText(""); setTradeIds([]); setEditId(null); setComposeOpen(false);
  };
  const byDate=useMemo(()=>{const m={};for(const n of notes){m[n.date]=m[n.date]||[];m[n.date].push(n)}return m},[notes]);

  const TradeChip=({t})=>{
    const v=computeDollarPnL(t,t.accType||"Dollar Account"); // accType not stored per trade; P&L display not required here; we compute relative
    const status = t.exitType && t.exitType!=="Trade In Progress" ? "CLOSED" : "OPEN";
    return (
      <div className="badge text-xs">
        <span className="badge-dot" style={{color: v>0?'#22c55e':v<0?'#ef4444':'#94a3b8'}}/>
        <span className="font-medium">{t.symbol}</span>
        <span>{t.strategy}</span>
        <span>{status}</span>
        <span>{v===null?'-':formatPnlDisplay("Dollar Account",v)}</span>
      </div>
    );
  };
  ...
  return(
    <div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-6">
      ...
      {composeOpen ? (
        <div className="ng-card ng-fade p-4">
                ...
                return (
                  <label key={t.id} className={`flex items-center gap-2 p-2 rounded-lg border ${checked?'border-blue-500 bg-slate-900':'border-slate-700 bg-slate-900/40'}`}>
                    <input type="checkbox" checked={checked} onChange={e=>{
                      const set=new Set(tradeIds); e.target.checked?set.add(t.id):set.delete(t.id); setTradeIds(Array.from(set));
                    }}/>
                    <TradeChip t={t}/>
                  </label>
                );
          ...
          <div className="mt-4 flex justify-between">
            <button onClick={()=>{setComposeOpen(false); setEditId(null); setText(""); setTradeIds([]);}} className="px-4 py-2 rounded-lg border border-slate-600 hover:bg-slate-700">Close</button>
            <div className="flex gap-2">
              {editId && <button onClick={()=>{
                setNotes(notes.filter(n=>n.id!==editId)); setEditId(null); setText(""); setTradeIds([]); setComposeOpen(false);
              }} className="px-4 py-2 rounded-lg border border-red-600 text-red-300 hover:bg-red-900/20">Delete</button>}
              <button onClick={saveNote} className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500">{editId?"Update":"Save"}</button>
            </div>
          </div>
        </div>
      ) : (
        <>
          <div className="mb-3 text-right">
            <button onClick={()=>setComposeOpen(true)} className="px-3 py-2 rounded-lg border border-slate-700 hover:bg-slate-800"><IconPlus/> Add note</button>
          </div>
          <div className="space-y-3">
            ...
            {notes.map(n=>(
              <div key={n.id} className="bg-slate-900/50 border border-slate-700 rounded-xl p-3">
                ...
                {n.tradeIds?.length>0 && <div className="mt-2 flex flex-wrap gap-2">
                  {n.tradeIds.map(id=>{const t=trades.find(x=>x.id===id); return t?<span key={id} className="badge text-xs"><span className="font-medium">{t.symbol}</span><span>{t.strategy}</span></span>:null})}
                </div>}
                <div className="mt-2"><button onClick={()=>{
                  setComposeOpen(true); setSelectedDate(n.date); setText(n.text); setTradeIds(n.tradeIds||[]); setEditId(n.id);
                }} className="text-blue-400">Open</button></div>
              </div>
            ))}
          </div>
        </>
...
/* ---------- Persisted state with normalization ---------- */
const normalizeState = (s,email) => ({
  name: s?.name || "",
  email: email || s?.email || "",
  accType: ACC_TYPES.includes(s?.accType) ? s.accType : ACC_TYPES[1],
  capital: Number.isFinite(s?.capital) ? s.capital : 0,
  depositDate: s?.depositDate || todayISO(),
  trades: Array.isArray(s?.trades) ? s.trades : [],
  notes: Array.isArray(s?.notes) ? s.notes : [],
  lastLotSize: Number.isFinite(s?.lastLotSize) ? s.lastLotSize : 0.01,
  customSymbols: Array.isArray(s?.customSymbols) && s.customSymbols.length ? s.customSymbols : DEFAULT_SYMBOLS,
  customStrategies: Array.isArray(s?.customStrategies) && s?.customStrategies.length ? s.customStrategies : DEFAULT_STRATEGIES,
  widgets: Array.isArray(s?.widgets) ? s.widgets : [...BUILTIN_WIDGETS],
  customWidgets: Array.isArray(s?.customWidgets) ? s.customWidgets : [],
  setupComplete: !!s?.setupComplete
});
function usePersisted(email){
  const fresh = () => normalizeState({email},email);
  const [state,setState]=useState(()=>{const s=loadState(email||getCurrent());return s?normalizeState(s,email||getCurrent()):fresh()});
  useEffect(()=>{const loaded = loadState(email); setState(loaded?normalizeState(loaded,email):fresh())}, [email]);
...
function StrategyWinRate({trades,accType,customStrategies}){
  ...
  const stats=useMemo(()=>{const m={}; customStrategies.forEach(s=>{m[s]={wins:0,losses:0}}); 
    ...
    return Object.entries(m).map(([s,{wins,losses}])=>({strategy:s,winRate:(wins+losses)>0?Math.round((wins/(wins+losses))*100):0})).filter(x=>x.winRate>0).sort((a,b)=>b.winRate-a.winRate)
  },[trades,accType,customStrategies]);
  if(stats.length===0) return null; // remain empty until a real best emerges
  const best=stats[0];
  return(<div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-4">
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="text-lg font-bold">{best.winRate}%</div>
    </div>
...
/* ---------- Header & Shell ---------- */
function Header({onToggleSidebar,onExport,onLogout,onImport}){
...
function AppShell({children,capitalPanel,nav,onToggleSidebar,onExport,onLogout,onImport,sidebarCollapsed}){
  return(<div className="min-h-screen">
    <Header onToggleSidebar={onToggleSidebar} onExport={onExport} onLogout={onLogout} onImport={onImport}/>
    <div className="flex">
...
function ResetModal({email,onClose}){
  ...
  const start=async ()=>{
    ...
    const first = (u.name||"").trim().split(" ")[0] || "Trader";
    const html = `
    ...
    try{
      ...
      setMsg(j.ok?"Reset email sent through Cloudflare.":"Reset request submitted.");
    }catch(err){
  ...
  return(<Modal title="Password reset" onClose={onClose} maxClass="max-w-md">
    <div className="space-y-3">
      ...
      <div className="text-xs text-slate-500">Note: This is a client-side app; the reset token is stored on the same device where the account was created.</div>
    </div>
  </Modal>);
}
...
function App(){
  ...
  const effectiveCapital=state.capital+realizedSum;
  ...
  function toCSV(rows,accType){
    ...
    for(const t of rows){
      ...
      const status = t.exitType && t.exitType!=="Trade In Progress" ? "CLOSED" : "OPEN";
      const row=[t.date,t.symbol,t.side,t.lotSize,(t.entry??""),(t.exit??""),(t.tp1??""),(t.tp2??""),(t.sl??""),t.strategy,(t.exitType||""),dollars,units,status];
  ...
  const onImport = (e) => {
    ...
    if (!file) return;
    setImporting("Importing… This may take a little while depending on file size.");
    const reader = new FileReader();
    reader.onload = (ev) => {
      try{
        let text = String(ev.target.result||"");
        const rows = parseCSV(text);
        if(rows.length<2){setImporting("No data rows found."); setTimeout(()=>setImporting(null),1200); return;}
        const headers = rows[0];
        const idx = importMapHeaders(headers);

        const out=[];
        for(let r=1;r<rows.length;r++){
          const row = rows[r]; if(row.every(x=>!x || String(x).trim()==="")) continue;
          const t={};
          t.date = readCell(row,idx.date) || todayISO();
          t.symbol = String(readCell(row,idx.symbol)||"").toUpperCase() || "XAUUSD";
          t.side = String(readCell(row,idx.side)||"BUY").toUpperCase()==="SELL"?"SELL":"BUY";
          t.lotSize = Number(readCell(row,idx.lotSize)) || 0.01;
          t.entry = Number(readCell(row,idx.entry));
          t.exit = Number(readCell(row,idx.exit));
          t.tp1 = Number(readCell(row,idx.tp1));
          t.tp2 = Number(readCell(row,idx.tp2));
          t.sl = Number(readCell(row,idx.sl));
          t.strategy = readCell(row,idx.strategy) || "Trend Line Bounce";
          const statusRaw = String(readCell(row,idx.status)||"").toUpperCase();
          t.exitType = readCell(row,idx.exitType);
          if(!t.exitType && statusRaw==="OPEN"){ t.exitType = "Trade In Progress"; }
          if(!t.exitType && statusRaw!=="OPEN"){ t.exitType = t.exit ? "TP" : ""; }

          // passthrough P&L if present
          const pnl = readCell(row,idx.pnl);
          if (typeof pnl === "number" && isFinite(pnl)) t.pnlProvided = pnl;

          t.id = Math.random().toString(36).slice(2);
          out.push(t);
        }
        // merge
        setState({...state, trades:[...out, ...state.trades]});
        setImporting("Imported "+out.length+" rows.");
        setTimeout(()=>setImporting(null),1200);
      }catch(err){
        setImporting("Import failed: "+err.message);
  ...
  const login=(email,password,setErr)=>{
    ...
    if(!u){
      if(password==="__google__"){
        ...
        const fresh=normalizeState({name:email.split("@")[0],email,widgets:[...BUILTIN_WIDGETS],setupComplete:false},email);
  ...
  const signup=(name,email,password,setErr)=>{
    ...
    const fresh=normalizeState({name,email,widgets:[...BUILTIN_WIDGETS],setupComplete:false},email);
  ...
  const renderDashboard=()=>{
    const pieces=[];
    const selected = state.widgets || [];
    if(selected.includes("GeneralStats")) pieces.push(<GeneralStats key="g" trades={state.trades} accType={state.accType} capital={state.capital} depositDate={state.depositDate}/>);
    if(selected.includes("StrategyWinRate")) pieces.push(<StrategyWinRate key="b" trades={state.trades} accType={state.accType} customStrategies={state.customStrategies}/>);
    if(selected.includes("DetailedStats")) pieces.push(<DetailedStats key="d" trades={state.trades} accType={state.accType}/>);
    // custom percentage widgets
    for(const w of state.customWidgets||[]){
      if(w.kind==="ExitTypeShare"){
        const closed=state.trades.filter(t=>t.exitType && t.exitType!=="Trade In Progress");
        const n=closed.length; const m=closed.filter(t=>String(t.exitType).toUpperCase()==="TP").length; // supports "TP" common case
        const pct=n?Math.round(100*m/n):0;
        pieces.push(<div key={w.id} className="bg-slate-800/60 border border-slate-700 rounded-2xl p-4"><div className="text-sm font-semibold mb-2">{w.title}</div><div className="text-lg font-bold">{pct}%</div><div className="mt-2 bg-slate-900/50 rounded-full h-2"><div className="bg-blue-600 h-2 rounded-full" style={{width:`${pct}%`}}/></div></div>);
      }
      if(w.kind==="StrategyWinRateFor"){
        const closed=state.trades.filter(t=>t.exitType && t.exitType!=="Trade In Progress" && t.strategy===w.arg);
        const wins=closed.filter(t=>{const v=computeDollarPnL(t,state.accType);return v>0}).length;
        const pct=closed.length?Math.round(100*wins/closed.length):0;
        pieces.push(<div key={w.id} className="bg-slate-800/60 border border-slate-700 rounded-2xl p-4"><div className="text-sm font-semibold mb-2">{w.title}</div><div className="text-lg font-bold">{pct}%</div><div className="mt-2 bg-slate-900/50 rounded-full h-2"><div className="bg-blue-600 h-2 rounded-full" style={{width:`${pct}%`}}/></div></div>);
      }
    }
    return <div className="space-y-4">{pieces}</div>;
  };

  const addCustomWidget = (w)=> setState({...state, customWidgets:[...(state.customWidgets||[]), w]});

  return(
    <AppShell capitalPanel={capitalPanel} nav={nav} onToggleSidebar={()=>setCollapsed(v=>!v)} onExport={onExport} onLogout={onLogout} onImport={onImport} sidebarCollapsed={collapsed}>
          ...
          <div className="overflow-auto"><table className="min-w-full text-sm">
            ...
            <tbody>{state.trades.map(t=>{
              ...
              return (<tr key={t.id} className="border-t border-slate-700">
                ...
                <Td>{t.exitType||""}</Td>
                <Td className={v>0?'text-green-400':v<0?'text-red-400':''}>{v===null?'-':formatPnlDisplay(state.accType,v)}</Td>
                <Td className={v>0?'text-green-400':v<0?'text-red-400':''}>{v===null?'-':formatUnits(state.accType,v)}</Td>
                <Td>{status}</Td>
                <Td><div className="flex gap-2">
                  <button onClick={()=>{setEditItem(t);setShowTrade(true)}} className="px-2 py-1 rounded-lg border border-slate-700 hover:bg-slate-700">✎</button>
                  <button onClick={()=>delTrade(t.id)} className="px-2 py-1 rounded-lg border border-red-700 text-red-300 hover:bg-red-900/20">✕</button>
                </div></Td>
              </tr>);
            })}</tbody></table></div>
        </div>
      ...
      {page==="settings" && (<SettingsPanel
        ...
        widgets={state.widgets} setWidgets={v=>setState({...state,widgets:v})}
        addCustomWidget={addCustomWidget}
      />)}
      ...
      {setupOpen && (
        <Modal title="Initial Account Setup" onClose={()=>{setSetupOpen(false); setState({...state, setupComplete:true})}} maxClass="max-w-3xl">
          <SettingsPanel
            name={state.name} setName={v=>setState({...state,name:v})}
            accType={state.accType} setAccType={v=>setState({...state,accType:v})}
            capital={state.capital} setCapital={v=>setState({...state,capital:v||0})}
            ...
            customStrategies={state.customStrategies} setCustomStrategies={v=>setState({...state,customStrategies:v})}
            widgets={state.widgets} setWidgets={v=>setState({...state,widgets:v})}
            addCustomWidget={(w)=>setState({...state,customWidgets:[...(state.customWidgets||[]),w]})}
            mode="setup"
            onDone={()=>{setSetupOpen(false); setState({...state, setupComplete:true})}}
          />
        </Modal>
      )}
      ...
      {importing && (
        <div className="ng-overlay">
          <div className="ng-card p-5 w-[90vw] max-w-md ng-fade">
            <div className="flex items-start gap-3">
              <IconInfo/>
              <div>
                <div className="font-semibold">Importing CSV</div>
                <div className="text-sm text-slate-300">{importing}</div>
                <div className="mt-2 bg-slate-900/50 rounded-full h-2 overflow-hidden"><div className="bg-blue-600 animate-pulse h-2 w-1/2 rounded-full"/></div>
              </div>
            </div>
          </div>
        </div>
