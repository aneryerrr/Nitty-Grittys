/* Nitty Gritty – tiny patch build (rev.6)
   - Cloud sync (Firestore) for Google sign-ins: your trades + account settings + strategies persist across devices.
   - Local behavior & UI unchanged (Reset History, Best Strategy, Settings icon, Forgot password preserved).
   - Robust import mapping kept. Notes preview width uniform.
   - Load reliability: fixed JSX className template issues; added versioned cache-busting in index.html.
*/
const {useState,useMemo,useEffect,useRef} = React;

/* ---------- Icons (unchanged look) ---------- */
...
/* Professional gear icon (approved) */
...
function computeDollarPnL(t,accType){
  ...
  switch(t.exitType){
    ...
    case"TP":if(has(tp2))return legPnL(t.symbol,t.side,entry,tp2,lot,accType);
             if(has(tp1))return legPnL(t.symbol,t.side,entry,tp1,lot,accType);return null;
...
/* CSV export (unchanged semantics) */
...
function TradeModal({initial,onClose,onSave,onDelete,accType,symbols,strategies}){
  ...
  const draft=useMemo(()=>({
    id:i.id,
    date,symbol,side,lotSize:parseFloat(lotSize||0),
    entry:num(entry),exit:num(exit),tp1:num(tp1),tp2:num(tp2),sl:num(sl),
    strategy,exitType,pnlOverride:(pnlOverride===""?undefined:parseFloat(pnlOverride))
  ...
  return(
    <Modal title={i.id?"Edit Trade":"Add Trade"} onClose={onClose} maxClass="max-w-4xl">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        <div><label className="text-sm text-slate-300">Action</label><div className="mt-1 grid grid-cols-2 gap-2">{["BUY","SELL"].map(s=>(
        ...
        <div><label className="text-sm text-slate-300">P&L Override ($)</label><input type="number" step="0.01" value={pnlOverride} onChange={e=>setPnlOverride(e.target.value)} className="w-full mt-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2" placeholder="Optional"/></div>
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={()=>onSave({...draft,id: i.id || Math.random().toString(36).slice(2)})} className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500">Save</button>
        </div>
      </div>
    </Modal>
  )
...
function CalendarModal({onClose,trades,view,setView,month,setMonth,year,setYear,selectedDate,setSelectedDate,accType}){
  ...
  const pnlByDate=useMemo(()=>{
    ...
    for(const date in byDate){
      const ts=byDate[date].filter(t=>t.exitType && t.exitType !== "Trade In Progress");
      const pnl=ts.reduce((a,t)=>a+ (computeDollarPnL(t,accType) || 0),0);
      m[date]=pnl;
    }
    return m;
  },[byDate,accType]);
    ...
    <Modal title="Calendar" onClose={onClose} maxClass="max-w-lg">
      <div className="flex items-center justify-between mb-3">
        ...
        {view!=="day"&&(
          <div className="flex items-center gap-2">
            <button onClick={()=>{
              if(view==='month'){ setMonth(m=>(m+11)%12); setYear(y=>y-(month===0?1:0)); }
              else setYear(y=>y-1);
            }} className="px-2 py-1 border border-slate-700 rounded-lg">&lt;</button>
            <div className="text-sm">{view==='month'?`${monthNames[month]} ${year}`:year}</div>
            <button onClick={()=>{
              if(view==='month'){ setMonth(m=>(m+1)%12); setYear(y=>y+(month===11?1:0)); }
              else setYear(y=>y+1);
            }} className="px-2 py-1 border border-slate-700 rounded-lg">&gt;</button>
          </div>
        )}
...
/* ---------- Dashboard blocks (incl. BestStrategy) ---------- */
...
function BestStrategy({trades,accType,strategies}){
  const data = useMemo(()=>{
    ...
    for(const t of trades){
      ...
      rec.count += 1; rec.pnl += v; if(v>0) rec.wins += 1; map.set(key,rec);
    }
...
/* ---------- Notes Panel (uniform preview width; rest unchanged) ---------- */
...
function ResetModal({email,onClose}){
  ...
  const start=async ()=>{
    ...
    const token=Math.random().toString(36).slice(2); const exp=Date.now()+1000*60*15;
...
/* ---------- Persisted local + Firestore cloud for Google ---------- */
...
function getFirst(normRow, candidates){ for(const c of candidates){ if(c in normRow){ const v = normRow[c]; if(v!=="" && v!==null && v!==undefined) return v; } } return undefined; }
...
/* ---------- Firebase Cloud Sync (Google sign-in only) ---------- */
const FIREBASE_CONFIG = {
  apiKey: "AIzaSyAS9y-WNQVW3sZfYKGDF_Ehm6NtIP2tv8o",
  authDomain: "nitty-gritty-3b1da.firebaseapp.com",
  projectId: "nitty-gritty-3b1da",
  storageBucket: "nitty-gritty-3b1da.firebasestorage.app",
  messagingSenderId: "425428858993",
  appId: "1:425428858993:web:f22633e6e6e96caa0a03d0",
  measurementId: "G-Q68NPS37HY"
};
const CLOUD = {
  app: null, db: null, authed: false,
  init(){
    if(!this.app){
  ...
  docFor(email){ this.init(); return this.db.collection('userState').doc(email); },
  async load(email){
    this.init();
    const snap = await this.docFor(email).get();
    return snap.exists ? snap.data() : null;
  },
  async save(email, payload){
    this.init();
    await this.docFor(email).set(payload, {merge:true});
  }
};
...
function App(){
  ...
  const now=new Date(); const [calView,setCalView]=useState("month");
  ...
  // Handle #reset= token for Forgot Password
  ...
  if (!__importEl.__ngBound){
    __importEl.addEventListener('change', async (e)=>{
      ...
      try{
        ...
        setState(s => ({ ...s, trades: [...trades.reverse(), ...s.trades] })); // prepend
      }catch(err){
  ...
  // Logout
  const onLogout=()=>{saveCurrent("");setCurrentEmail(""); try{firebase.auth().signOut()}catch(e){} };

  // Google init + Firebase Auth link
  const initGoogle=(container,onEmail)=>{
    ...
    window.google.accounts.id.initialize({
      ...
      callback: async (resp)=>{
        ...
        if(p && p.email){ // Sign in to Firebase with the same Google credential (for Firestore security rules)
          try{ await CLOUD.signInWithGoogleIdToken(resp.credential); }catch(e){ console.warn("Firebase Google sign-in failed:", e); }
          onEmail(p.email);
        }
      }
    });
  ...
  // Local login/signup (unchanged), but after Google login we also cloud-sync
  const login=async (email,password,setErr)=>{
    ...
    if(!u){
      if(password==="__google__"){
        ...
        // Cloud load/seed
        try{
          const cloud = await CLOUD.load(email);
          if(cloud && cloud.state){
            saveState(email,cloud.state);
            saveCfg(email,cloud.cfg||{symbols:DEFAULT_SYMBOLS,strategies:DEFAULT_STRATEGIES});
            // reflect instantly
            setState(cloud.state);
            setCfg(cloud.cfg||{symbols:DEFAULT_SYMBOLS,strategies:DEFAULT_STRATEGIES});
          }else{
            await CLOUD.save(email,{state:fresh,cfg:{symbols:DEFAULT_SYMBOLS,strategies:DEFAULT_STRATEGIES}});
          }
        }catch(e){ console.warn("Cloud sync (first login) failed:", e); }
        return;
      }
    ...
    // If already Firebase authed via Google, try load cloud
    try{
      if(CLOUD.userEmail()===u.email){
        const cloud = await CLOUD.load(u.email);
        if(cloud && cloud.state){
          saveState(u.email,cloud.state);
          saveCfg(u.email,cloud.cfg||{symbols:DEFAULT_SYMBOLS,strategies:DEFAULT_STRATEGIES});
          setState(cloud.state); setCfg(cloud.cfg||{symbols:DEFAULT_SYMBOLS,strategies:DEFAULT_STRATEGIES});
        }
      }
    }catch(e){ console.warn("Cloud fetch after login failed:", e); }
  };
  const signup=(name,email,password,setErr)=>{
    ...
    saveState(email,fresh); saveCurrent(email); setCurrentEmail(email);
    setCfg({symbols:DEFAULT_SYMBOLS,strategies:DEFAULT_STRATEGIES});
    // (optional) not linking to cloud unless they use Google sign-in
  };
  const resetStart=()=>{setShowReset(true)};

  // CRUD trades
  const addOrUpdate=(rec)=>{
    const arr=state.trades.slice(); const idx=arr.findIndex(t=>t.id===rec.id);
    const next = (idx>=0 ? (arr[idx]=rec, arr) : [rec, ...arr]);
    setState({...state,trades:next});
  ...
  // Debounced cloud auto-save for Google accounts
  useEffect(()=>{
    if(!currentEmail) return;
    if(CLOUD.userEmail()!==currentEmail) return; // only when Google + Firebase auth matches current email
    const payload = { state, cfg };
    const t = setTimeout(()=>{ CLOUD.save(currentEmail, payload).catch(e=>console.warn("Cloud save failed:", e)); }, 700);
    return ()=>clearTimeout(t);
  }, [state, cfg, currentEmail]);

  if(resetToken){
    return <NewPasswordModal token={resetToken} onClose={()=>{setResetToken(""); location.hash=""}}/>
  }
  if(!currentEmail){
    return (<>
