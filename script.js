...
const toISO = (v) => {
  ...
  const d=new Date(s);
  if(!isNaN(d)) { const t=new Date(d.getTime()-d.getTimezoneOffset()*60000); return t.toISOString().slice(0,10); }
  return todayISO();
};
...
function perLotValueForMove(symbol,delta,accType){
  const abs=Math.abs(delta); const isStd=accType==="Dollar Account";
  const mult=usd=>isStd?usd:usd/100;
  switch(symbol){
...
function importCSVText(text, settings, accType){
  const rows=parseCSVRaw(text); if(!rows.length) return [];
  ...
  for(let r=1;r<rows.length;r++){
    const row=rows[r]; const get=i=> i>-1 ? row[i] : "";
...
/* -------------------- Modal -------------------- */
const Modal=({title,children,onClose,maxClass})=>(
...
function CustomizeJournal({settings,setSettings,availableSymbols,setAvailableSymbols,availableStrategies,setAvailableStrategies}){
  ...
  const onDrop=(e,idx)=>{
    ...
    const arr=[...settings.dashboardOrder]; const [m]=arr.splice(from,1); arr.splice(idx,0,m);
  ...
  const removeFromDashboard=(key)=>setSettings({...settings, dashboardOrder: settings.dashboardOrder.filter(k=>k!==key)});
        ...
        <div className="space-y-2">
          {settings.dashboardOrder.map((key,idx)=>(
            <div key={key}
              ...
              <div className="flex items-center gap-2"><IconGrip className="cursor-move"/><div className="text-sm">{key}</div></div>
...
function TradeModal({initial,onClose,onSave,onDelete,accType,symbols,strategies}){
  ...
  const [lotSize,setLotSize]=useState(i.lotSize??0.01); // moved directly below title
...
/* -------------------- Notes (date picker, rich text, checklist, ref-trades filtered by date) -------------------- */
  ...
  return(
    <Modal title="New Note" onClose={onClose} maxClass="max-w-3xl">
      ...
      <div className="flex gap-2 mb-2">
        ...
        <select className="toolbar-select" onChange={e=>exec("fontSize", e.target.value)}>{[1,2,3,4,5,6,7].map(n=><option key={n} value={n}>Size {n}</option>)}</select>
          ...
          {notes.map(n=>{
            ...
            return (
              <div key={n.id} className="bg-slate-900/50 border border-slate-700 rounded-xl p-3 note-card">
                ...
                <div className="text-sm leading-6 max-h-36 overflow-auto" dangerouslySetInnerHTML={{__html:n.html}} />
...
/* -------------------- Login View (both sides dark-blue) -------------------- */
function LoginView({onLogin,onSignup,initGoogle,resetStart}){
  ...
  return(
    <div className="min-h-screen grid md:grid-cols-2 bg-deep">
...
function App(){
  ...
  const useFresh=(email)=>({
    ...
    trades: [],
    notes: [],
    settings: { dashboardOrder:["general","best","detailed"], customBoxes:[], strategyColors:{} },
  ...
  const onImport=()=>{const input=document.createElement("input");input.type="file";input.accept=".csv,text/csv";input.onchange=async e=>{const file=e.target.files[0];if(!file) return;setImporting({visible:true,progress:5,msg:"Reading file…"});const text=await file.text();setImporting({visible:true,progress:35,msg:"Parsing rows…"});const newTrades=importCSVText(text,state,state.accType);setImporting({visible:true,progress:65,msg:"Merging…"});const merged=[...newTrades.map(t=>({...t,id:Math.random().toString(36).slice(2)})),...state.trades];setState({...state,trades:merged});setTimeout(()=>setImporting({visible:false,progress:100,msg:""}),600)};input.click();};
  ...
  const dashDragHandlers=(idx)=>({
    ...
    onDragOver:(e)=>{ e.preventDefault(); },
    onDrop:(e)=>{ e.preventDefault(); if(dragIdx===null||dragIdx===idx) return;
  ...
  return(
    <AppShell capitalPanel={capitalPanel} nav={nav} logoSrc={logoSrc} onToggleSidebar={()=>setCollapsed(v=>!v)} onExport={onExport} onImport={onImport} onLogout={onLogout} sidebarCollapsed={collapsed}>
      ...
      {page==="settings"&&(
        <CustomizeJournal 
          settings={state.settings} setSettings={(s)=>setState({...state,settings:s})}
          availableSymbols={state.availableSymbols} setAvailableSymbols={(s)=>setState({...state,availableSymbols:s})}
          availableStrategies={state.availableStrategies} setAvailableStrategies={(s)=>setState({...state,availableStrategies:s})}
        />
      )}
      ...
      {importing.visible && (
        <Modal title="Importing CSV…" onClose={()=>setImporting({visible:false,progress:0,msg:""})} maxClass="max-w-md">
          <div className="space-y-3">
            <div className="text-sm text-slate-300">Please wait. Large files can take a few minutes.</div>
...
function ResetModal({email,onClose}){
  const [e,setE]=useState(email||""); const [msg,setMsg]=useState(""); const [link,setLink]=useState("");
  const start=async ()=>{
    ...
    try{
      await emailjs.send('YOUR_SERVICE_ID','YOUR_TEMPLATE_ID',{to_email:e,first_name:u.name.split(' ')[0],reset_link:url,expiry_time:"15 minutes"});
      setMsg("Reset email sent.");
    }catch(err){ setMsg("Failed to send email. (EmailJS not configured)"); }
  };
  return(
