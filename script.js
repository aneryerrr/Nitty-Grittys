...
const IconSettings=(p)=>(<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} className={iconCls} {...p}><circle cx="12" cy="12" r="3.5"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06A1.65 1.65 0 0 0 15 19.4M8 4.6A1.65 1.65 0 0 0 9 4h.1"/></svg>);
...
const IconNotes=(p)=>(<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} className={iconCls} {...p}><rect x="4" y="4" width="16" height="16" rx="2"/><path d="M8 8h8M8 12h8M8 16h5"/></svg>);
const IconEdit=(p)=>(<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className={iconCls} {...p}><path d="M15 3l6 6M3 21l3-9 9-9 6 6-9 9-9 3"/></svg>);
const IconTrash=(p)=>(<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className={iconCls} {...p}><path d="M3 6h18"/><path d="M8 6V4h8v2"/><rect x="6" y="6" width="12" height="14" rx="2"/><path d="M10 11v6M14 11v6"/></svg>);
const IconX=(p)=>(<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className={iconCls} {...p}><path d="M18 6L6 18M6 6l12 12"/></svg>);
...
/* ---------- CSV helpers (fixed robust parser) ---------- */
function parseCSV(text){
  if(!text) return [];
  // strip BOM
  if(text.charCodeAt(0)===0xFEFF){ text=text.slice(1); }
  const rows=[]; let field=""; let row=[]; let i=0; let q=false;
  while(i<text.length){
    const c=text[i];
    if(q){
  ...
  return(<div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-3">
    <div className={`relative w-[95vw] ${maxClass||"max-w-3xl"} max-h-[80vh] overflow-auto bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl`}>
      <div className="sticky top-0 z-10 flex items-center justify-between px-4 py-3 border-b border-slate-700 bg-slate-800/95 backdrop-blur">
        ...
        <button onClick={onClose} className="px-3 py-1.5 rounded-lg border border-slate-600 hover:bg-slate-700"><IconX/></button>
      </div>
...
function SettingsPanel({
  ...
  widgets,setWidgets, forSetup=false, onSetupDone
}){
  const [tab,setTab]=useState(forSetup?"account":"account");
  ...
  const [symToRemove,setSymToRemove]=useState("");
  const [strToRemove,setStrToRemove]=useState("");
  ...
  return(
    <div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-4"><IconSettings/><div className="font-semibold">{forSetup?"Account Setup":"Settings"}</div></div>
      ...
      {/* Welcome text in setup */}
      {forSetup && (
        <div className="mb-4 text-slate-300">
          <div className="text-lg font-semibold mb-1">Welcome to Nitty Gritty 👋</div>
          <div className="text-sm">Let’s set up your account basics. You can edit everything later in Settings.</div>
        </div>
      )}

      {(tab==="account" || forSetup) && (
        <div className="space-y-4">
          ...
          {forSetup && (
            <div className="text-right">
              <button onClick={onSetupDone} className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500">Start journaling</button>
            </div>
          )}
        </div>
      )}

      {!forSetup && tab==="security"&&(
      ...
      {!forSetup && tab==="customize"&&(
        <div className="grid md:grid-cols-2 gap-6">
          <div>
            <div className="text-sm font-semibold mb-2">Symbols</div>
            {/* Dropdown to remove */}
            <div className="flex gap-2 mb-3">
              <select value={symToRemove} onChange={e=>setSymToRemove(e.target.value)} className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2">
                <option value="">— Select symbol to remove —</option>
                {(customSymbols||[]).map(s=><option key={s} value={s}>{s}</option>)}
              </select>
              <button onClick={()=>{ if(symToRemove){ setCustomSymbols((customSymbols||[]).filter(x=>x!==symToRemove)); setSymToRemove(""); }}} className="px-3 py-2 rounded-lg border border-red-600 text-red-300 hover:bg-red-900/20">Remove</button>
            </div>
            {/* Add new */}
            <div className="flex gap-2">
              ...
              <button onClick={()=>{ if(sym){ setCustomSymbols([...(customSymbols||[]),sym.toUpperCase()]); setSym(""); }}} className="px-3 py-2 rounded-lg bg-blue-600 hover:bg-blue-500">Add</button>
            </div>
          </div>
          ...
          <div>
            ...
            {/* Dropdown to remove */}
            <div className="flex gap-2 mb-3">
              <select value={strToRemove} onChange={e=>setStrToRemove(e.target.value)} className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2">
                <option value="">— Select strategy to remove —</option>
                {(customStrategies||[]).map(s=><option key={s} value={s}>{s}</option>)}
              </select>
              <button onClick={()=>{ if(strToRemove){ setCustomStrategies((customStrategies||[]).filter(x=>x!==strToRemove)); setStrToRemove(""); }}} className="px-3 py-2 rounded-lg border border-red-600 text-red-300 hover:bg-red-900/20">Remove</button>
            </div>
            {/* Add new */}
            <div className="flex gap-2">
          ...
          {/* Also allow widget toggles here (requested) */}
    ...
    <Modal title={i.id?"Edit Trade":"Add Trade"} onClose={onClose} maxClass="max-w-4xl">
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {i.id&&(<button onClick={()=>onDelete(i.id)} className="px-4 py-2 rounded-lg border border-red-600 text-red-300 hover:bg-red-900/20"><IconTrash/></button>)}
          <button onClick={onClose} className="px-4 py-2 rounded-lg border border-slate-600 hover:bg-slate-700"><IconX/></button>
...
/* Notes (new composer-first flow, smaller tab, enhanced trade reference UI) */
function NotesPanel({notes,setNotes,trades,initialOpen=false}){
  ...
  const allNotes = useMemo(()=>[...notes].sort((a,b)=> (b.createdAt||0)-(a.createdAt||0)),[notes]);

  const tradeMap=useMemo(()=>{
    const m={}; for(const t of trades){ const pnl=computeDollarPnL(t,"Dollar Account"); m[t.id]={...t,pnl}; } return m;
  },[trades]);

  const save=()=>{
    const id=editing||Math.random().toString(36).slice(2);
    const rec={id,date,text,tradeId:tradeId||null,createdAt:Date.now()};
    const arr=notes.slice(); const i=arr.findIndex(n=>n.id===id);
    if(i>=0) arr[i]={...rec}; else arr.unshift(rec);
    setNotes(arr);
    // reset composer & close
    setText(""); setTradeId(""); setEditing(null); setOpen(false);
  };
  const discard=()=>{ setText(""); setTradeId(""); setEditing(null); setOpen(false); };
    ...
    return (
      <div className="badge">
        <span className="inline-flex items-center gap-1">
          <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M3 12h18M12 3v18"/></svg>
          {t.symbol} · {t.side} · Lot {t.lotSize}
        </span>
        <span className={`ml-2 text-xs ${closed?'text-green-300':'text-yellow-300'}`}>{closed?'CLOSED':'OPEN'}</span>
        {typeof t.pnl==="number" && <span className={`ml-2 text-xs ${t.pnl>0?'text-green-400':t.pnl<0?'text-red-400':'text-slate-300'}`}>{fmt$(t.pnl)}</span>}
      </div>
    );
  };

  return(
    <div className="max-w-3xl mx-auto bg-slate-800/60 border border-slate-700 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2"><IconNotes/><div className="font-semibold">Notes</div></div>
        {!open && <button onClick={()=>{setOpen(true); setDate(todayISO());}} className="px-3 py-2 rounded-lg border border-slate-700 hover:bg-slate-800"><IconPlus/> New note</button>}
      </div>
        ...
        <div className="bg-slate-900/50 border border-slate-700 rounded-xl p-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <div className="md:col-span-3">
              ...
              <textarea value={text} onChange={e=>setText(e.target.value)} className="w-full mt-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 h-40" placeholder="Write your observation..."/>
            </div>
          </div>
          ...
          <div className="mt-3">
            ...
            <div className="mt-1 flex gap-2">
              <select value={tradeId} onChange={e=>setTradeId(e.target.value)} className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2">
                <option value="">— None —</option>
                {trades.map(t=><option key={t.id} value={t.id}>{`${t.date} · ${t.symbol} · ${t.side}`}</option>)}
              </select>
              {tradeId && <button onClick={()=>setTradeId("")} className="px-3 py-2 rounded-lg border border-slate-700 hover:bg-slate-800"><IconX/></button>}
            </div>
            <div className="mt-2">{tradeId && <TradeChip t={tradeMap[tradeId]} />}</div>
          </div>

          <div className="mt-4 flex justify-between">
            <button onClick={discard} className="px-4 py-2 rounded-lg border border-slate-600 hover:bg-slate-700"><IconX/> Close</button>
            <button onClick={save} className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500">{editing?"Update":"Save note"}</button>
          </div>
        </div>
      ) : (
        // List view with previews
        <div className="space-y-2">
          {allNotes.length===0 && <div className="text-slate-400 text-sm">No notes yet. Click “New note”.</div>}
          {allNotes.map(n=>{
            const t= n.tradeId ? tradeMap[n.tradeId] : null;
            return (
              <div key={n.id} className="bg-slate-900/50 border border-slate-700 rounded-xl p-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={()=>{ setOpen(true); setDate(n.date); setText(n.text); setTradeId(n.tradeId||""); setEditing(n.id); }} className="px-2 py-1 rounded-lg border border-slate-700 hover:bg-slate-800"><IconEdit/></button>
                    <button onClick={()=>del(n.id)} className="px-2 py-1 rounded-lg border border-red-700 text-red-300 hover:bg-red-900/20"><IconTrash/></button>
                  </div>
                </div>
                <div className="mt-1 text-sm line-clamp-2">{n.text||"(empty)"}</div>
                {t && <div className="mt-2"><TradeChip t={t}/></div>}
              </div>
            );
          })}
        </div>
        ...
        return(<tr key={t.id} className="border-t border-slate-700">
          ...
          <Td>
            <div className="flex gap-2">
              ...
              <button onClick={()=>onDelete(t.id)} className="px-2 py-1 rounded-lg border border-red-700 text-red-300 hover:bg-red-900/20"><IconTrash/></button>
            </div>
          </Td>
        </tr>)})}</tbody>
    </table></div>
...
function Header({logoSrc,onToggleSidebar,onExport,onLogout,onImport}){
  return(<div className="sticky top-0 z-30 flex items-center justify-between px-4 py-3 border-b border-slate-800 bg-slate-950/70 backdrop-blur">
    <div className="flex items-center gap-3">
      <button onClick={onToggleSidebar} className="px-3 py-2 rounded-lg border border-slate-700 hover:bg-slate-800"><svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M3 6h18M3 12h18M3 18h18"/></svg></button>
      <div className="flex items-center gap-3">
  ...
  return(<div className="min-h-screen grid md:grid-cols-2">
    <div className="hidden md:flex hero items-center justify-center">
      <div className="max-w-sm text-center px-6">
        ...
        <div className="mt-3 text-slate-200">“Discipline is choosing what you want most over what you want now.”</div>
      </div>
    </div>
...
/* ---------- Setup modal (welcome + account only) ---------- */
function SetupModal({state,setState,onClose}){
  return (
    <Modal title="Welcome" onClose={onClose} maxClass="max-w-2xl">
      <SettingsPanel
        ...
        email={state.email}
        customSymbols={state.customSymbols||[]} setCustomSymbols={()=>{}}
        customStrategies={state.customStrategies||[]} setCustomStrategies={()=>{}}
...
function App(){
  ...
  // Open setup for brand-new accounts (welcome)
  useEffect(()=>{
    if(state && state.email && !state.setupComplete){
      setShowSetup(true);
    }
  },[state?.email]);
  ...
  /* Import — uses robust parseCSV (fixed) */
  const onImport=(e)=>{
    ...
    reader.onload=(ev)=>{
      try{
        ...
        const col = {
          ...
          lot: idx('lot size','lots','qty','lot'),
          entry: idx('entry','entry price','entryprice'),
          ...
          exitType: idx('exit type','outcome','status'),
          pnl: idx('p&l','p&l ($)','pnl'),
        ...
        for(let r=1;r<rows.length;r++){
          const raw=rows[r]; if(!raw || raw.every(c=>String(c||"").trim()==="")) continue;
          const get=(ix)=> ix>=0 && ix<raw.length ? String(raw[ix]).replace(/^"|"$/g,'').replace(/""/g,'"').trim() : undefined;
          const t={
            id: Math.random().toString(36).slice(2),
            date: get(col.date) || todayISO(),
            symbol: (get(col.symbol)||'N/A').toUpperCase(),
            side: (get(col.side)||'BUY').toUpperCase().includes('SELL')?'SELL':'BUY',
        ...
        setState({...state,trades:[...state.trades,...imported]});
      } finally {
        setImporting(false);
      }
    };
  ...
  const login=(email,password,setErr)=>{
    ...
    if(!u){
      if(password==="__google__"){ // auto-provision
        ...
        const fresh={name:email.split("@")[0],email,accType:ACC_TYPES[1],capital:0,depositDate:todayISO(),trades:[],notes:[],lastLotSize:0.01,customSymbols:[...DEFAULT_SYMBOLS],customStrategies:[...DEFAULT_STRATEGIES],widgets:{showGeneral:true,showBest:false,showDetailed:true},setupComplete:false};
  ...
  const signup=(name,email,password,setErr)=>{
    ...
    const fresh={name,email,accType:ACC_TYPES[1],capital:0,depositDate:todayISO(),trades:[],notes:[],lastLotSize:0.01,customSymbols:[...DEFAULT_SYMBOLS],customStrategies:[...DEFAULT_STRATEGIES],widgets:{showGeneral:true,showBest:false,showDetailed:true},setupComplete:false};
  ...
  return(
    <>
      ...
      <AppShell capitalPanel={capitalPanel} nav={nav} logoSrc={logoSrc}
        ...
        {page==="notes"&&(<NotesPanel initialOpen={true} notes={state.notes} setNotes={notes=>setState({...state,notes})} trades={state.trades}/>)}
      ...
      {showSetup && (
        <SetupModal
          state={state}
          setState={setState}
          onClose={()=>{ setState({...state, setupComplete:true}); setShowSetup(false); }}
        />
      )}
    </>
  )
